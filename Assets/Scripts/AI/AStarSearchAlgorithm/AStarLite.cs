...
{
    private readonly int gridSizeX = 72;
    private readonly int gridSizeY = 40;
    private readonly float cellSize = 2;
    private AStarNode[,] aStarNodes;
    private AStarNode startNode;
    private List<AStarNode> nodesToCheck = new();
    private readonly List<AStarNode> nodesChecked = new();
    private List<Vector2> aiPath = new();

    //Debug 
    private Vector3 startPositionDebug = new(1000, 0, 0);
    private Vector3 destinationPositionDebug = new(1000, 0, 0);
        ...
        for (int x = 0; x < gridSizeX; x++)
            for (int y = 0; y < gridSizeY; y++)
            {
                ...
                if (hitCollider2D != null)
                {
                    ...
                    */
                    //Mark as obstacle
                    hitCollider2D.transform.IsChildOf(transform);
                    if (hitCollider2D.transform.CompareTag("Obstacle"))
    ...
    public List<Vector2> FindPath(Vector2 destination)
    {
        ...
        aiPath = CreatePathForAI();

        return aiPath;
    }

    private List<Vector2> CreatePathForAI()
    {
        List<Vector2> resultAIPath = new();
        List<AStarNode> aiPath = new();
    ...
    private Vector2Int ConvertWorldToGridPoint(Vector2 position)
    {
        ...
        Vector2Int gridPoint = new(Mathf.RoundToInt(position.x / cellSize + gridSizeX / 2.0f), Mathf.RoundToInt(position.y / cellSize + gridSizeY / 2.0f));
