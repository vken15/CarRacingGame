...
public class SpawnCars : NetworkBehaviour
{
    ...
    private void SetSpawnCarInfo(GameObject car, GameObject nameplate, Driver driver, int i)
    {
        ...
        else
        {
            ...
            if (driver.DriverInput == InputType.mouse)
            {
                car.GetComponent<CarInputHandler>().enabled = false;
                CarAIHandler carAI = car.GetComponent<CarAIHandler>();
                carAI.aiMode = CarAIHandler.AIMode.followMouse;
                carAI.isAvoidingCars = false;
            }
            else
            {
